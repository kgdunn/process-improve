...
- **A complexity budget with a ratchet (#307).** `tools/complexity_budget.py`
  ...
  `smooth_trajectories` and #581's `equal_var` switch each added one. #208 is
  then net neutral: `_pareto_front` adds a `PLR0913` breach and the
  `to_spec` simplification removes a `C901` one.
