...
- ``"tsr"`` (trimmed score regression, the default): regress the model's
  scores on the trimmed scores computed from the observed columns. The
  regression is fitted on the training rows, so it knows how much the
  observed columns have actually said about each score in the past, and
  shrinks toward the mean batch while they say little. That is what makes it
  the strongest of the three in both papers' comparisons, and the inverted
  matrix is only (A x A). It needs the training residual block; see
  ``x_residuals`` in :func:`operator_for_pattern`.
- ``"pmp"`` (projection to the model plane): least-squares fit of the
  observed columns onto the corresponding loading rows. It uses no training
  information beyond the plane itself, so early in a batch, with few columns
  observed, it is free to place the row anywhere on the plane that fits them:
  the estimates are unbiased but can be wildly out of scale, and the operator
  is often ill-conditioned. For a PLS model the plane is spanned by the
  loadings while the scores are generated by the weights, so PMP does not
  reduce to the model's own scores even when nothing is missing; TSR and SCP
  do.
- ``"scp"`` (single-component projection): estimate each component
  ...
  errors propagate through the deflation, and it too uses no information
  about how the training scores co-varied.
...
def _residual_inner(x_residuals: np.ndarray | None, observed: np.ndarray, g_obs: np.ndarray) -> np.ndarray:
    """Build the residual half of the TSR inner matrix, ``G*' E*' E* G* / (n_train - 1)``.

    Returns a zero matrix, with a warning, when the training residuals were not
    supplied: the caller then gets the plane-only matrix, which is not TSR.
    """
    A = g_obs.shape[1]
    if x_residuals is None:
        warnings.warn(
            "method='tsr' was asked for without the training residuals (x_residuals), so its inner matrix "
            "keeps only the model-plane term. That is the projection-to-the-model-plane estimator, not "
            "trimmed score regression. Pass the fitted model's residual block, or ask for method='pmp' "
            "explicitly.",
            SpecificationWarning,
            stacklevel=3,
        )
        return np.zeros((A, A))
    residuals = np.asarray(x_residuals, dtype=float)
    if residuals.ndim != 2 or residuals.shape[1] != observed.shape[0]:
        raise ValueError(
            f"x_residuals must be (n_train, {observed.shape[0]}) over the model's features; "
            f"got shape {residuals.shape}."
        )
    n_train = residuals.shape[0]
    # E* G*: (n_train x A). Never form the (n_observed x n_observed) Gram.
    projected = residuals[:, observed] @ g_obs
    return projected.T @ projected / max(1, n_train - 1)
...
def operator_for_pattern(  # noqa: PLR0913 - the operator inputs are irreducible
    ...
    *,
    ...
    x_residuals: np.ndarray | None = None,
) -> Bunch:
    ...
    x_residuals : np.ndarray of shape (n_train, n_features), optional
        The training residual block ``E = Z - T P'`` in the model's centred
        and scaled space, used by ``tsr`` only. The TSR regression is on the
        covariance of the *observed* columns, which is the model plane
        ``P* Theta P*'`` plus the spread around it ``E*'E* / (n_train - 1)``;
        dropping the second term is what turns TSR into ``pmp`` (exactly so,
        for a PCA). Omitting it therefore raises a
        :class:`~process_improve.multivariate._common.SpecificationWarning`
        and falls back to the plane-only matrix. A fitted PCA / PLS carries
        this block, and its ``project`` / ``projection_matrix`` methods pass
        it for you.
    ...
    if method == "pmp":
        ...
        # Arteaga and Ferrer (2002): t_hat = Theta P*' G* [G*' S** G*]^-1 G*' z*,
        # with S** the covariance of the observed columns in the training data.
        # S** is the model plane plus the residual spread around it; the second
        # term is what carries "how much have these columns told us so far",
        # and without it the whole expression collapses to the least-squares
        # fit onto the plane (for a PCA, bitwise the ``pmp`` operator).
        # G*' E*' E* G* is formed as the Gram of the (n_train x A) product
        # E* G*, so nothing of size (n_observed x n_observed) is ever built.
        inner = g_obs.T @ p_obs @ theta @ p_obs.T @ g_obs + ridge * np.eye(A)
        inner = inner + _residual_inner(x_residuals, observed, g_obs)
...
def project_rows(  # noqa: PLR0913 - mirrors operator_for_pattern
    ...
    *,
    ...
    x_residuals : np.ndarray of shape (n_train, n_features), optional
        As in :func:`operator_for_pattern`: the training residual block that
        ``tsr`` regresses with.
