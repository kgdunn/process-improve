...
BUDGET: dict[str, int] = {
    ...
    "C901": 46,
    "PLR0912": 26,
    # 81, and none of the three moves is a regression this branch introduced.
    ...
    # valid keys depend on `method`. #581 then gave `ttest_independent_from_df`
    # its `equal_var` switch, taking it to six, which is the honest width for two
    # columns, a frame and three switches. This branch adds `_pareto_front`,
    # whose models, goals, naming and search region are four separate inputs.
    # All three carry a reasoned suppression, which is the discipline this
    # ratchet exists to enforce, so the budget tracks what is really there
    # rather than pretending the count did not move.
    "PLR0913": 81,
    "PLR0915": 31,
}
